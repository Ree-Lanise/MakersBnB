...
def test_find_by_email(db_connection):
    ...
    user = repository.find_by_email('Aakash@outllok.com')
    assert user == User(2, 'Aakash', 'P*ssword', 'Aakash@outllok.com')
...
def test_check_password(db_connection):
    ...
    repo.create(User(None, 'John', '1236758Pass', 'John@outlook.com'))
    
    # we use the check password function with the original password attempt
    result = repo.check_password('John@outlook.com', '1236758Pass')
...
def test_check_password_false(db_connection):
    ...
    # we use the check password function with an incorrect or FAKE password 
